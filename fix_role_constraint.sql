-- Fix performed_by_role check constraint issue
-- Check what role values are allowed

-- 1. Check the constraint on ticket_history.performed_by_role
SELECT 
...
-- 2. Check what performed_by_role values currently exist
SELECT DISTINCT performed_by_role 
FROM public.ticket_history 
ORDER BY performed_by_role;

-- 3. Update the log_timeline_event function to use valid role values
...
BEGIN
    ...
    BEGIN
        ...
        -- Map roles to valid performed_by_role values
        CASE p_performed_by_role
            ...
            WHEN NULL THEN valid_role := 'system'; -- Default for system events
            ELSE valid_role := 'system'; -- Default fallback
        END CASE;
...
-- 4. Test the function with valid role
